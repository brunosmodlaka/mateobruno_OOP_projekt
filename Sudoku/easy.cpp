...
void startEasy(sf::RenderWindow& window, const sf::Font& uiFont, const sf::Font& gridFont)
{
    bool solved = false;
    bool gameOver = false;

    window.setTitle("EASY: usao u startEasy");
    ...
    auto res = puzzle::loadFromFile("easy_puzzles.txt");

    if (!res.error.empty()) {
        window.setTitle("EASY ERROR: " + res.error);
        showLoadError(window);
        return;
    }
    if (res.puzzles.empty()) {
        window.setTitle("EASY ERROR: puzzles.empty()");
        showLoadError(window);
        return;
    }
