...
void startMedium(sf::RenderWindow& window, const sf::Font& uiFont, const sf::Font& gridFont)
{
    bool solved = false;
    bool gameOver = false;

    window.setTitle("medium: usao u startmedium");
    ...
    auto res = puzzle::loadFromFile("medium_puzzles.txt");

    if (!res.error.empty()) {
        window.setTitle("medium ERROR: " + res.error);
        showLoadError(window);
        return;
    }
    if (res.puzzles.empty()) {
        window.setTitle("medium ERROR: puzzles.empty()");
        showLoadError(window);
        return;
    }
